# Exercício 31: Continuação da introdução ao Pandas através da utilização de comandos adicionais do DataFrame.
...
10. Por meio da função "describe" é possível ter acesso a dados estatísticos, como: 
    a. Número de valores contidos na lista;
    b. A média desses valores;
    c. O desvio padrão;
    d. O valor mínimo identificado;
    e. Primeiro quartis (valor que separa os 25% menores do restante);
    f. Segundo quartis (mediana);
    g. Terceiro quartis (valor que separa os 75% menores do restante);
...
print(df["tipo"])   # Exibe somente a coluna "tipo";
print(df["valor"].mean())   # Realiza a média dos valores;
print(df.describe())    # Resumo estatístico completo.